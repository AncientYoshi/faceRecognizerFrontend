...
export class SystemService {
  ...
  audit(
    ...
    return this.http.get<PageResponse<AuditLog>>(`${environment.apiUrl}/audit-logs`, { params });
  }
      ...
      return of([
        {
          ...
          value: '0.85',
          valueType: 'DECIMAL',
          description: 'Minimum accepted face similarity',
          updatedBy: null,
          updatedAt: new Date().toISOString(),
        },
      ]);
    return this.http.get<SystemSetting[]>(`${environment.apiUrl}/system-settings`);
  }
  setting(key: string): Observable<SystemSetting> {
    if (this.auth.isPreview())
      return of({
        id: 'preview',
        key,
        value: '0.85',
        valueType: 'DECIMAL',
        description: 'Preview setting',
        updatedBy: null,
        updatedAt: new Date().toISOString(),
      });
    return this.http.get<SystemSetting>(
      `${environment.apiUrl}/system-settings/${encodeURIComponent(key)}`,
    );
  }
