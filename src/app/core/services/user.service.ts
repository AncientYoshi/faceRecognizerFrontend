...
const PREVIEW_USERS: UserSummary[] = [
  ...
  {
    id: 'u5',
    email: 'noah.williams@sam.edu',
    firstName: 'Noah',
    lastName: 'Williams',
    enabled: true,
    roles: ['STUDENT'],
    studentId: 's2',
    studentNumber: 'STU-24019',
    studyYear: 2,
    createdAt: '2026-04-12T09:00:00Z',
  },
  {
    id: 'u6',
    email: 'ava.brown@sam.edu',
    firstName: 'Ava',
    lastName: 'Brown',
    enabled: false,
    roles: ['STUDENT'],
    studentId: 's3',
    studentNumber: 'STU-24020',
    studyYear: 2,
...
export class UserService {
  ...
  list(query = '', page = 0, size = 10): Observable<PageResponse<UserSummary>> {
    if (this.auth.isPreview()) {
      ...
      const filtered = this.previewUsers().filter(
        (user) =>
          !normalized ||
          `${user.firstName} ${user.lastName} ${user.email} ${user.studentNumber ?? ''} ${user.employeeNumber ?? ''}`
            .toLowerCase()
            .includes(normalized),
      );
  ...
  create(payload: CreateUserPayload): Observable<UserSummary> {
    if (this.auth.isPreview()) {
      const user: UserSummary = {
        ...
        studyYear: payload.studyYear ?? undefined,
        employeeNumber: payload.employeeNumber ?? undefined,
        createdAt: new Date().toISOString(),
        studentId: payload.roles.includes('STUDENT') ? crypto.randomUUID() : undefined,
        teacherId: payload.roles.includes('TEACHER') ? crypto.randomUUID() : undefined,
      };
