...
export class StudentTimetableComponent implements OnInit {
  ...
  load() {
    ...
    this.error.set('');
    this.api.mine(this.week()).subscribe({
      next: (x) => {
        this.week.set(x.weekStart);
        this.timetable.set(x);
      ...
      error: (e) => {
        ...
        this.error.set(
          e.status === 0
            ...
            : e.error?.message || 'Your timetable could not be loaded.',
        );
      },
    });
  }
  move(days: number) {
    ...
    this.week.set(this.localDate(d));
    this.load();
  }
  choose(value: string) {
    if (!value) return;
    this.week.set(this.monday(new Date(`${value}T00:00:00`)));
    this.load();
  }
  currentWeek() {
    this.week.set(this.monday(new Date()));
    this.load();
  }
  trackEntry(_: number, entry: StudentTimetableEntry) {
    return entry.timetableId;
  }

  private monday(date: Date) {
