...
export class StudentDashboardComponent implements OnInit {
  ...
  readonly meetsThreshold = computed(
    () => this.overallPercentage() >= (this.dashboard()?.requiredAttendancePercentage || 0),
  );
  readonly hasAttendance = computed(() => (this.dashboard()?.eligibleSessions || 0) > 0);
  readonly progressStyle = computed(
    () => `${Math.min(100, Math.max(0, this.overallPercentage()))}%`,
  );
      ...
      error: (error) => {
        this.error.set(
          error.status === 0
            ...
            : error.error?.message || 'Your dashboard could not be loaded.',
        );
        this.loading.set(false);
      },
    });
    this.attendance.listMyAttendance().subscribe({
      next: (page) => this.records.set(page.content),
