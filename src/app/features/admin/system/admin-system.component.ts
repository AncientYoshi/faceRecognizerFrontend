...
export class AdminSystemComponent implements OnInit {
  private api = inject(SystemService);
  private fb = inject(FormBuilder);
  readonly mode = inject(ActivatedRoute).snapshot.data['mode'] as Mode;
  ...
  load() {
    ...
    if (this.mode === 'settings')
      this.api.settings().subscribe({
        next: (x) => {
          this.settings.set(x);
          this.drafts.set(Object.fromEntries(x.map((s) => [s.key, s.value])));
      ...
      this.api.audit(filters).subscribe({
        next: (x) => {
          this.logs.set(x.content);
          this.loading.set(false);
        },
        error: (e) => this.fail(e),
      });
  ...
  draft(key: string, value: string) {
    this.drafts.update((x) => ({ ...x, [key]: value }));
  }
  save(setting: SystemSetting) {
    this.saved.set('');
    this.api.updateSetting(setting.key, this.drafts()[setting.key]).subscribe({
      next: (x) => {
        this.settings.update((all) => all.map((s) => (s.key === x.key ? x : s)));
        this.saved.set(x.key);
      },
      error: (e) => this.fail(e),
    });
  }
  private fail(e: any) {
    ...
    this.error.set(
      e.status === 0
        ...
        : e.error?.message || 'The request could not be completed.',
    );
  }
}
