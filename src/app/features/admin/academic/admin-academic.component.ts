...
export class AdminAcademicComponent implements OnInit{
  ...
  form=this.fb.nonNullable.group({code:['',Validators.required],name:['',Validators.required],description:[''],semester:['FIRST',Validators.required],academicYear:['2026-2027',Validators.required],departmentId:['',Validators.required],teacherId:['',Validators.required],studentId:['',Validators.required]});
  ...
  openCreate(){this.editing.set(null);this.form.reset({code:'',name:'',description:'',semester:'FIRST',academicYear:'2026-2027',departmentId:this.departments()[0]?.id||'',teacherId:this.teachers()[0]?.teacherId||'',studentId:this.students()[0]?.studentId||''});this.modal.set(true)}
  openEdit(item:Department|Course){this.editing.set(item);if(this.mode==='departments'){const d=item as Department;this.form.patchValue({code:d.code,name:d.name,description:d.description})}else{const c=item as Course;this.form.patchValue({code:c.code,name:c.name,semester:c.semester,academicYear:c.academicYear,departmentId:c.departmentId,teacherId:c.teacherId})}this.modal.set(true)}
  ...
  private courseBody(v:any):CoursePayload{return{code:v.code,name:v.name,semester:v.semester,academicYear:v.academicYear,departmentId:v.departmentId,teacherId:v.teacherId}}
