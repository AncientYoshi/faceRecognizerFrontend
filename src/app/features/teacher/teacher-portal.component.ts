...
import { Enrollment, TeacherDashboard } from '../../core/models/api.models';
...
export class TeacherPortalComponent implements OnInit {
  ...
  readonly mode = this.route.snapshot.data['mode'] as 'dashboard' | 'courses';
  ...
  ngOnInit() {
    this.dashboards.getTeacherDashboard().subscribe({
      next: (x) => {
        ...
        if (this.mode === 'courses' && x.myCourses.length) this.select(x.myCourses[0].courseId);
      },
      error: (e) => {
        this.error.set(e.error?.message || 'Could not load the teacher portal.');
        this.loading.set(false);
      },
    });
  }
  select(courseId: string) {
    ...
    this.academics.enrollments(courseId, 0, 100).subscribe({
      next: (x) => this.enrollments.set(x.content),
      error: (e) => this.error.set(e.error?.message || 'Could not load course students.'),
    });
  }
}
